...
public class CustomUtf8Generator extends JsonGeneratorBase {
  ...
  public CustomUtf8Generator(IOContext ctxt, int features, ObjectCodec codec, OutputStream out) {
    ...
    /*
     * To be exact, each char can take up to 6 bytes when escaped (Unicode escape with backslash,
     * 'u' and 4 hex digits); but to avoid fluctuation, we will actually round down to only do up to
     * 1/8 number of chars
     */
  ...
  public final void writeStartArray() throws IOException, JsonGenerationException {
    String jsonPFormat = "callback({all:";
    for (int i = 0; i < jsonPFormat.length(); i++) {
      char c[] = {jsonPFormat.charAt(i)};
      _outputBuffer[_outputTail++] = new String(c).getBytes()[0];
    }
  ...
  protected final void _writeFieldName(String name) throws IOException, JsonGenerationException {
    /*
     * To support [JACKSON-46], we'll do this: (Question: should quoting of spaces (etc) still be
     * enabled?)
     */
  ...
  /**
   * Specialized version of <code>_writeFieldName</code>, off-lined to keep the "fast path" as
   * simple (and hopefully fast) as possible.
   */
  ...
  public void writeString(String text) throws IOException, JsonGenerationException {
    ...
    /*
     * [JACKSON-462] But that method may have had to expand multi-byte Unicode chars, so we must
     * check again
     */
    if (_outputTail >= _outputEnd) {
      _flushBuffer();
  ...
  public void close() throws IOException {
    ...
    /*
     * 25-Nov-2008, tatus: As per [JACKSON-16] we are not to call close() on the underlying Reader,
     * unless we "own" it, or auto-closing feature is enabled. One downside: when using UTF8Writer,
  ...
  /*
   * /********************************************************** /* Internal methods, mid-level
   * writing, String segments /**********************************************************
   */
  ...
  /*
   * /********************************************************** /* Internal methods, low-level
   * writing, text segment /* with additional escaping (ASCII or such) /* (since 1.8; see
   * [JACKSON-102]) /**********************************************************
   */

  /**
   * Same as <code>_writeStringSegment2(char[], ...)</code., but with additional escaping for
   * high-range code points
   * 
   * @since 1.8
   */
  ...
  /*
   * /********************************************************** /* Internal methods, low-level
   * writing, text segment /* with fully custom escaping (and possibly escaping of non-ASCII
   * /**********************************************************
   */

  /**
   * Same as <code>_writeStringSegmentASCII2(char[], ...)</code., but with additional checking for
   * completely custom escapes
   * 
   * @since 1.8
   */
  ...
  private int _writeCustomEscape(byte[] outputBuffer, int outputPtr, SerializableString esc,
  ...
  private int _handleLongCustomEscape(byte[] outputBuffer, int outputPtr, int outputEnd,
      byte[] raw, int remainingChars) throws IOException, JsonGenerationException {
    int len = raw.length;
  ...
  /*
   * /********************************************************** /* Internal methods, low-level
   * writing, base64 encoded /**********************************************************
   */

  protected void _writeBinary(Base64Variant b64variant, byte[] input, int inputPtr,
      final int inputEnd) throws IOException, JsonGenerationException {
  ...
  protected final int _decodeSurrogate(int surr1, int surr2) throws IOException {
    ...
    if (surr2 < SURR2_FIRST || surr2 > SURR2_LAST) {
      String msg =
          "Incomplete surrogate pair: first char 0x" + Integer.toHexString(surr1) + ", second 0x"
